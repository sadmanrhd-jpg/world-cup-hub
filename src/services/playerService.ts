...
const CACHE_KEY = "fan26.world-cup-player-data-v2";
...
const mapPosition = (value: string): PlayerPosition | null => {
  ...
  if (position.includes("DEFENDER") || position.endsWith("DF") || position.endsWith("DEF")) return "DEF";
  if (position.includes("MIDFIELDER") || position.endsWith("MF") || position.endsWith("MID")) return "MID";
  if (position.includes("FORWARD") || position.endsWith("FW") || position.endsWith("FWD") || position.endsWith("ST")) return "FWD";
  return null;
};
...
const teamByHeading = (value: string) => {
  ...
  return TEAMS.find(
    (team) => team.slug === slug || normalize(team.name) === key || normalize(team.slug) === key,
  );
};

const playerId = (teamSlug: string, name: string) => `${teamSlug}:${normalize(name)}`;

const mapStats = (value: Partial<PlayerTournamentStats> | undefined): PlayerTournamentStats => ({
  ...emptyStats(),
  ...value,
...
const readCache = (): CachedData | null => {
  ...
  try {
    const value = JSON.parse(localStorage.getItem(CACHE_KEY) ?? "null") as CachedData | null;
...
const writeCache = (players: WorldCupPlayer[], managers: WorldCupManager[]) => {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(
      CACHE_KEY,
      JSON.stringify({ storedAt: Date.now(), players, managers } satisfies CachedData),
    );
  } catch {
...
const parseWikipediaInBrowser = async (): Promise<SquadPayload> => {
  ...
  const payload = await fetchJson<{ parse?: { text?: string } }>(endpoint.toString());
  if (!payload.parse?.text) throw new Error("Wikipedia returned no squad page HTML.");

  const documentNode = new DOMParser().parseFromString(payload.parse.text, "text/html");
  ...
  for (const heading of Array.from(documentNode.querySelectorAll("h3"))) {
    ...
    const headingContainer = heading.parentElement?.classList.contains("mw-heading")
      ? heading.parentElement
      : heading;
    let cursor = headingContainer.nextElementSibling;
    while (cursor) {
      if (cursor.matches("h2, h3") || cursor.querySelector(":scope > h2, :scope > h3")) break;
      section.appendChild(cursor.cloneNode(true));
      cursor = cursor.nextElementSibling;
    }

    const coachParagraph = Array.from(section.querySelectorAll("p")).find((paragraph) =>
      /^(?:Head coach|Coach)\s*:/i.test(clean(paragraph.textContent ?? "")),
    );
    const coachLinks = coachParagraph ? Array.from(coachParagraph.querySelectorAll("a")) : [];
    const linkedCoach = clean(coachLinks[coachLinks.length - 1]?.textContent ?? "");
    const textCoach = clean(coachParagraph?.textContent ?? "").replace(
      /^(?:Head coach|Coach)\s*:\s*/i,
      "",
    );
    managerNames.set(team.slug, linkedCoach || textCoach || getManager(team.name));
...
const loadData = async (): Promise<CachedData> => {
  ...
  if (cached && Date.now() - cached.storedAt < CACHE_TTL) return cached;
  ...
  try {
    ...
    const statPayload: StatsPayload = await fetchJson<StatsPayload>(
      "/api/world-cup-stats",
    ).catch((): StatsPayload => ({ stats: {} }));
    ...
    writeCache(value.players, value.managers);
    return value;
  } catch (error) {
